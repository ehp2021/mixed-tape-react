...
const AlbumPage = () => {
    ...
    return ( 
      <Wrapper>
        <Header />
        <New_Wrapper> { /* <div className="artist-name">{tracks[0].artists[0].name}</div> */ } { /* {album ? <Image src={album.images[1].url}/> : null} */ } {
            album ? < div className = 'album-cover'
            style = {
                { backgroundImage: `url(${album.images[1].url})` }
            } > </div>: null} <div style = {
                { alignSelf: "center", fontSize: "15px" }
            } > Release Date: { album ? album.release_date : "" } </div> {
            tracks.map((track, i) => {
                return ( <Content>
                    <li >
